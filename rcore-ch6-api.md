...
`easy-fs-fuse` 和内核共用本实验的 `easy-fs`。尚未完成的骨架会在宿主机构建镜像、首次调用 `Inode::create` 时触发 TODO，构建流程在进入 QEMU 之前中断，这是骨架阶段的预期现象。完成 easy-fs 部分后，内核加载初始进程仍依赖 OS 层的 `open_file`、`read_all` 等接口；完成十四处 TODO 后才具备运行全部现有测例的条件。
