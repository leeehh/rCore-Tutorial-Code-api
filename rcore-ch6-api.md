...
1. **代码修改范围**：仅允许修改 `ch6-api` 分支中 [easy-fs/src/vfs.rs](easy-fs/src/vfs.rs) 的 `Inode::find/create/ls/read_at/write_at/clear/stat/link/unlink`，以及 [os/src/fs/inode.rs](os/src/fs/inode.rs) 的 `open_file`、`OSInode::read_all` 和 `File for OSInode` 中的 `read/write/stat`，共十四处 TODO；内部辅助函数可在这两个文件中设计。保留已有数据结构、接口和配套函数语义，不修改其他代码文件、磁盘布局、构建配置或测试文件；可以新增实验报告等说明文档。

2. **静态分析与动态跟踪**：阅读 `ch6` 分支中文件与目录管理的参考实现，使用 GDB 观察文件打开、内容读取等关键路径，并静态分析创建、截断、硬链接及回收的调用关系。操作参考 [ch6 源代码分析与动态跟踪文档](../../blob/ch6/rcore-ch6-analyze.md)。报告应区分文件描述符、打开对象、磁盘 inode 和目录项，记录实际断点、命令、状态与结论，可附必要源码片段或链接。API 配套 `exec` 已补充栈和堆边界重置，应保留该提供实现，不将差异扩展为进程代码修改任务。

3. **独立实现与对比**：依据本文契约独立完成十四处接口，并按“运行与验收”小节执行验收。在报告中对比自己的实现与 `ch6` 参考实现的偏移管理、缓存同步、目录项复用、链接计数和回收策略，说明相同点、不同点及实现原因；保持本文的文件名、资源足够及最后链接删除后不继续使用旧句柄等约定。

4. **主要问题与解决思路**：总结主要问题的现象、原因、排查过程、解决思路及处理结果，结合代码位置、GDB 记录或运行输出说明依据。关注锁重入、缓存块借用和 inode 与数据块的资源归属；未解决的问题如实记录。

实验报告采用 Markdown 格式，保存到仓库根目录的 `reports/lab6.md`，至少包含静态分析与动态跟踪、独立实现对比、主要问题与解决思路三部分，并随实验代码提交。参考实现上的 GDB 跟踪用于分析，完成后的功能验收仍使用本文规定的命令和既有测例。
...
功能验收只使用这一已有测试入口，不新增测例或额外验收命令。实现完成后，内核应启动到用户终端，现有测例应完成文件打开、读写、创建、截断、硬链接和元数据检查，以及所调用的往章功能。
