...
2. **静态分析与动态跟踪**：阅读 `ch8` 分支的同步与死锁检测参考实现，使用 GDB 跟踪互斥请求和安全性判断，并静态分析阻塞、唤醒、信号量与条件变量的调用关系。操作参考 [ch8 源代码分析与动态跟踪文档](../../blob/ch8/rcore-ch8-analyze.md)。报告应记录实际断点、命令、资源与线程状态及结论，可附必要源码片段或链接；区分负信号量计数、非负可用资源和线程当前的一项请求。
...
4. **主要问题与解决思路**：总结主要问题的现象、原因、排查过程、解决思路及处理结果，结合代码位置、GDB 记录或运行输出说明依据。关注调度前释放借用、唤醒前交付资源、恢复后重复记账和关闭检测仍需记账等问题；未解决的问题如实记录。
