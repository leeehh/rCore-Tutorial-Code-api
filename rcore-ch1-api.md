...
1. **代码修改范围**：仅允许修改 `ch1-api` 分支中 [os/src/main.rs](os/src/main.rs) 的 `rust_main()` 函数体，保留已有入口属性、函数签名和链接符号声明，不修改同文件中的 `clear_bss()` 等配套代码。不得修改其他任何代码文件、链接脚本、构建配置或测试文件；可以新增实验报告等说明文档。

2. **静态分析与动态跟踪**：阅读 `ch1` 分支中 `os/src/main.rs` 的参考实现，并使用 GDB 动态跟踪 `rust_main()` 的执行过程，分析其初始化、输出和退出等功能与本文任务描述的对应关系。具体操作参考 [ch1 源代码分析与动态跟踪文档](../../blob/ch1/rcore-ch1-analyze.md)。报告应结合源代码说明调用顺序及其原因，并记录关键断点、使用的 GDB 命令、观察到的状态和分析结论。引用源代码时，可附必要的代码片段或源码链接，便于核对。参考实现未包含本实验要求的局部数组求和与结果输出，应在报告中明确指出这部分新增任务。

3. **独立实现与对比**：依据本文接口契约，在 `ch1-api` 分支独立完成 `rust_main()` 的全部任务，并按“运行与验收”小节执行验收。在报告中对比自己的实现与 `ch1` 参考实现的相同点和不同点，说明各自对应的任务要求，以及采用相关实现方式的原因；对于本实验新增的计算任务，也应说明实现方法。对比应围绕实际功能、执行顺序和实现思路展开，不能只列出代码文本的差异。

4. **主要问题与解决思路**：总结实验中遇到的主要问题，描述问题现象、原因分析、排查过程、解决思路及处理结果。结合相关代码位置、GDB 跟踪记录或运行输出说明依据；尚未解决的问题也应如实记录。

实验报告采用 Markdown（`.md`）格式，统一保存到仓库根目录下的 `reports/lab1.md`，至少包含上述静态分析与动态跟踪、独立实现对比、主要问题与解决思路三部分，并随实验代码提交。GDB 动态跟踪用于参考实现分析，完成后的功能验收仍使用本文规定的运行命令。
...
功能验收只使用上述命令，不新增测试脚本或额外验收命令；参考实现的 GDB 动态跟踪按“实验要求”完成并记入报告。RustSBI 的启动输出之后，应能观察到：
